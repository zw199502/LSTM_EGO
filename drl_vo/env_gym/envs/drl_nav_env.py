...
class DRLNavEnv(gym.Env):
    def __init__(self):
        ...
        self.action_deviation_factor = None
        ...
        self.robot_policy = policy_factory[self.human_policy_name]()
        self.robot_policy.safety_space = 0.15
        self.robot_policy.time_step = 0.2
    ...
    def configure(self, reward_simple=False):
        ...
        self.action_deviation_factor = 0.1
    ...
    def step(self, action):
        # generate robot action with orca
        ...
        action_orca = self.robot_policy.predict(ob_coordinate)
        ...
        if not done:
            reward = reward + self.goal_distance_factor * (self.goal_distance_last - goal_dist) \
                            - self.action_deviation_factor * (fabs(action[0] - action_orca[0]) + fabs(action[1] - action_orca[1]))
