...
class CrowdSim:
    def __init__(self, args):
        ...
        self.actor_change_penalty_factor = 0.05
    ...
    def generate_random_human_position(self):
        ...
        if self.static_obstacle_num == 0 and self.human_num == 0:
            self.human_num = 1
        self.steady_human_num = np.random.randint(self.human_num + 1,  size=1)[0]
        self.humans = []
        if self.human_num > 0:
            for i in range(self.human_num - self.steady_human_num):
                self.humans.append(self.generate_circle_crossing_human())

            for i in range(self.steady_human_num):
                self.humans.append(self.generate_steady_human())
    ...
    def generate_steady_human(self):
        human = Human()
        ...
        while True:
            px = np.random.uniform(self.robot.px + human.radius , self.robot.gx - human.radius, size=1)[0]
    ...
    def reset(self, phase='test'):
        ...
        ob_position_last_action = np.array([r, t, self.action_last[0] / self.robot.v_pref, self.action_last[1] / self.robot.v_pref], dtype=np.float32)
        ...
        return self.scan_current, ob_position_last_action
    ...
    def step(self, _action):
        ...
        for i in range(self.human_num - self.steady_human_num):
            ...
            human_actions.append(self.humans[i].act(ob))

        for i in range(self.steady_human_num):
            human_actions.append(np.zeros(2))

       

        action = _action * self.delta_action + self.action_last
        action = np.clip(action, -self.robot.v_pref, self.robot.v_pref)

        self.action_last[0] = action[0]
        self.action_last[1] = action[1]
        ...
        return self.scan_current, ob_position_last_action, reward, done, info
