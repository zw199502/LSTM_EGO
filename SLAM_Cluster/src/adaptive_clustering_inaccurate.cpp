...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2_in) {
  ...
  pt.setInputCloud(pcl_pc_in_transformed);
  pt.setFilterFieldName("y");
  pt.setFilterLimits(-1.2, 1.2);
  ...
  for(int i = 0; i < pc_indices->size(); i++) {
    float range = 0.0;
    float d2 = pcl_pc_in_transformed->points[(*pc_indices)[i]].x * pcl_pc_in_transformed->points[(*pc_indices)[i]].x +
               pcl_pc_in_transformed->points[(*pc_indices)[i]].y * pcl_pc_in_transformed->points[(*pc_indices)[i]].y +
               pcl_pc_in_transformed->points[(*pc_indices)[i]].z * pcl_pc_in_transformed->points[(*pc_indices)[i]].z;
  ...
  for(int i = 0; i < region_max_; i++) {
    ...
    if(indices_array[i].size() > cluster_size_min_) {
      ...
      tree->setInputCloud(pcl_pc_in_transformed, indices_array_ptr);
      ...
      ec.setInputCloud(pcl_pc_in_transformed);
      ...
      for(std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); it++) {
      	...
      	for(std::vector<int>::const_iterator pit = it->indices.begin(); pit != it->indices.end(); ++pit) {
      	  cluster->points.push_back(pcl_pc_in_transformed->points[*pit]);
  	    }
  ...
  if(cloud_filtered_pub_.getNumSubscribers() > 0) {
    ...
    pcl::copyPointCloud(*pcl_pc_in_transformed, *pc_indices, *pcl_pc_out);
